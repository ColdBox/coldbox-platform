...
<input type="hidden" name="#primaryKey#" value="%rc.o#root.bean.xmlAttributes.name#bean.get#primaryKey#()%" />
	...
	<fieldset>
	<legend>#root.bean.xmlAttributes.name# Editor : <%cfif rc.o#root.bean.xmlAttributes.name#bean.get#primaryKey#() eq 0 %>Add<%cfelse%>Edit<%/cfif%> Record
	<legend>

	<table border="0" cellpadding="5" cellspacing="0" class="tablelisting">
	<cfloop from="1" to="#arrayLen(root.bean.dbtable.xmlChildren)#" index="i">
		...
		<tr>
			<td><label for="#element.name#"><cfif element.required eq "Yes"><em>*</em></cfif>#element.name#:</label></td>
			...
			<input id="#element.name#" name="#element.name#" type="text" 
				   <%cfif %rc.o#root.bean.xmlAttributes.name#bean.get#element.name#()% eq ""%>value="%now()%"<%cfelse%>value="%rc.o#root.bean.xmlAttributes.name#bean.get#element.name#()%"<%/cfif%> size="40"  />
			<cfelse>
			<input id="#element.name#" name="#element.name#" type="text" value="%rc.o#root.bean.xmlAttributes.name#bean.get#element.name#()%" maxlength="#element.length#" size="40"  />
			</cfif>
			</td>
		</tr>
		</cfif>		
