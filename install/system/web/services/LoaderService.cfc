...
01/18/2007 - Created
----------------------------------------------------------------------->
<cfcomponent name="LoaderService" output="false" hint="The application and framework loader service" extends="shared.frameworks.coldbox_3_00.system.web.services.BaseService">
...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<!--- Get App Loader --->
	<cffunction name="getAppLoader" access="public" returntype="shared.frameworks.coldbox_3_00.system.web.loader.AbstractApplicationLoader" output="false" hint="Get the application configuration loader used">
	...
	<cffunction name="loadApplication" returntype="void" access="Public" hint="I load a coldbox application for operation." output="false">
		...
		<!--- ************************************************************* --->
		<cfscript>
		var debuggerConfig = createObject("Component","shared.frameworks.coldbox_3_00.system.web.config.DebuggerConfig").init();
	...
	<!--- createDefaultLogBox --->
    <cffunction name="createDefaultLogBox" output="false" access="public" returntype="shared.frameworks.coldbox_3_00.system.logging.LogBox" hint="Create a running LogBox instance configured using ColdBox's default config">
    	<cfscript>
   		var logBoxConfig = "";
		
		logBoxConfig = createObject("component","shared.frameworks.coldbox_3_00.system.logging.config.LogBoxConfig").init(CFCConfigPath="shared.frameworks.coldbox_3_00.system.web.config.LogBox");
		
		return createObject("component","shared.frameworks.coldbox_3_00.system.logging.LogBox").init(logBoxConfig,controller);
    	</cfscript>
    </cffunction>
    ...
    <cffunction name="createWireBox" output="false" access="public" returntype="void" hint="Create WireBox DI Framework with config settings.">
    	<cfscript>
			...
			if( controller.getCFMLEngine().isMT() AND wireboxData.enabled ){
				oInjector = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.Injector").init(wireboxData.binderPath,controller.getConfigSettings(), controller);
	...
	<!--- createCacheBox --->
    <cffunction name="createCacheBox" output="false" access="public" returntype="shared.frameworks.coldbox_3_00.system.cache.CacheFactory" hint="Create the application's CacheBox instance">
    	<cfscript>
    		var config 				= createObject("Component","shared.frameworks.coldbox_3_00.system.cache.config.CacheBoxConfig").init();
			...
			// Create CacheBox
			return createObject("component","shared.frameworks.coldbox_3_00.system.cache.CacheFactory").init(config,controller);
		</cfscript>
    </cffunction>
    ...
    <cffunction name="createCacheManager" output="false" access="public" returntype="any" hint="Create the compatibility caching engine">
    	<cfscript>
		// Create cache Config
		var cacheConfig = createObject("Component","shared.frameworks.coldbox_3_00.system.cache.archive.config.CacheConfig");
		var cache = "";
		...
		// Create according cache manager
   		if ( controller.getCFMLEngine().isMT() ){
			cache = CreateObject("component","shared.frameworks.coldbox_3_00.system.cache.archive.MTCacheManager").init(controller);
		}
		else{
			cache = CreateObject("component","shared.frameworks.coldbox_3_00.system.cache.archive.CacheManager").init(controller);
		}
...
<!------------------------------------------- PRIVATE ------------------------------------------->
	...
	<!--- createAppLoader --->
	<cffunction name="createAppLoader" output="false" access="private" returntype="shared.frameworks.coldbox_3_00.system.web.loader.AbstractApplicationLoader" hint="Detect the application loader to use and create it">
		...
		if( listLast(coldboxSettings["ConfigFileLocation"],".")  eq "cfc" ){
			instance.appLoader = createObject("component","shared.frameworks.coldbox_3_00.system.web.loader.CFCApplicationLoader").init(controller);
			return instance.appLoader;
		}
		
		// Return XML Loader
		instance.appLoader = createObject("component","shared.frameworks.coldbox_3_00.system.web.loader.XMLApplicationLoader").init(controller);
		return instance.appLoader;
		</cfscript>
	</cffunction>
