...
01/18/2007 - Created
----------------------------------------------------------------------->
<cfcomponent output="false" hint="The ColdBox exception service" extends="shared.frameworks.coldbox_3_00.system.web.services.BaseService">
...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<!--- exception handler --->
	<cffunction name="exceptionHandler" access="public" hint="I handle a framework/application exception. I return a framework exception bean" returntype="any" output="false" colddoc:generic="shared.frameworks.coldbox_3_00.system.web.context.ExceptionBean">
		<!--- ************************************************************* --->
		...
		<cfscript>
		var bugReport 		= "";
		var exceptionBean 	= createObject("component","shared.frameworks.coldbox_3_00.system.web.context.ExceptionBean").init(errorStruct=arguments.exception,extramessage=arguments.extraMessage,errorType=arguments.errorType);
		...
		if ( len(controller.getSetting("ExceptionHandler")) ){
			try{
				...
				exceptionBean = createObject("component","shared.frameworks.coldbox_3_00.system.web.context.ExceptionBean").init(errorStruct=e,extramessage="Error Running Custom Exception handler",errorType="application");
				// Log it
				appLogger.logErrorWithBean(exceptionBean);
			}
		}
		...
		<cfif Exception.getErrortype() eq "application" and controller.getSetting("CustomErrorTemplate") neq "">
			<cftry>
				...
				<cfcatch type="any">
					...
					<!--- Save the Bug Report --->
					<cfsavecontent variable="cboxBugReport"><cfinclude template="/shared/frameworks/coldbox_3_00/system/includes/BugReport.cfm"></cfsavecontent>
				</cfcatch>
			</cftry>
		<cfelse>
			<!--- Save the Bug Report --->
