...
<!----------------------------------------- CONSTRUCTOR ------------------------------------->			
	...
	<!--- init --->
	<cffunction name="init" access="public" returntype="Builder" hint="Constructor. If called without a configuration binder, then WireBox will instantiate the default configuration binder found in: shared.frameworks.coldbox_3_00.system.ioc.config.DefaultBinder" output="false" >
		...
		<cfscript>
			...
			if( instance.injector.isColdBoxLinked() ){
				instance.coldboxDSL = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.dsl.ColdBoxDSL").init( arguments.injector );
			}
			// Is CacheBox Linked?
			if( instance.injector.isCacheBoxLinked() ){
				instance.cacheBoxDSL = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.dsl.CacheBoxDSL").init( arguments.injector );
			}
			return this;
		</cfscript>
	</cffunction>
	...
	<cffunction name="getProviderDSL" access="private" returntype="any" hint="Get dependencies using the our provider pattern DSL" output="false" >
		...
		<cfscript>
			...
			if( instance.injector.containsInstance( providerName ) ){
				...
				return createObject("component","shared.frameworks.coldbox_3_00.system.ioc.Provider").init(instance.injector, providerName);
			}
			else if( instance.log.canDebug() ){
