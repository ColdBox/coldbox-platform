...
Description :
	...
	Easy Startup:
	injector = new shared.frameworks.coldbox_3_00.system.ioc.Injector();
	
	Binder Startup
	injector = new shared.frameworks.coldbox_3_00.system.ioc.Injector(new MyBinder());
	
	Binder Path Startup
	injector = new shared.frameworks.coldbox_3_00.system.ioc.Injector("config.MyBinder");
...
<!----------------------------------------- CONSTRUCTOR ------------------------------------->			
	...
	<!--- init --->
	<cffunction name="init" access="public" returntype="Injector" hint="Constructor. If called without a configuration binder, then WireBox will instantiate the default configuration binder found in: shared.frameworks.coldbox_3_00.system.ioc.config.DefaultBinder" output="false" >
		<cfargument name="binder" 		required="false" default="shared.frameworks.coldbox_3_00.system.ioc.config.DefaultBinder" hint="The WireBox binder or data CFC instance or instantiation path to configure this injector with">
		...
		<cfargument name="coldbox" 		required="false" default="" hint="A coldbox application context that this instance of WireBox can be linked to, if not using it, we just ignore it." colddoc:generic="shared.frameworks.coldbox_3_00.system.web.Controller">
		<cfscript>
			// Setup Available public scopes
			this.SCOPES = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.Scopes");
			// Setup Available public types
			this.TYPES = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.Types");
			
			// Do we have a binder?
			if( NOT len(trim(arguments.binder)) ){ arguments.binder = "shared.frameworks.coldbox_3_00.system.ioc.config.DefaultBinder"; }
			...
			instance = {
				...
				// Utility class
				utility  = createObject("component","shared.frameworks.coldbox_3_00.system.core.util.Util"),
				// Version
				version  = "1.0.0",	 
	...
	<cffunction name="configure" output="false" access="public" returntype="void" hint="Configure this injector for operation, called by the init(). You can also re-configure this injector programmatically, but it is not recommended.">
		<cfargument name="binder" 		required="true" hint="The configuration binder object or path to configure this Injector instance with" colddoc:generic="shared.frameworks.coldbox_3_00.system.ioc.config.Binder">
		...
		<cflock name="#instance.lockName#" type="exclusive" timeout="30" throwontimeout="true">
			...
			instance.builder = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.Builder").init( this );
    ...
    <cffunction name="buildInstance" output="false" access="public" returntype="any" hint="Build an instance, this is called from registered scopes only as they provide locking and transactions">
    	<cfargument name="mapping" 			required="true" 	hint="The mapping to construct" colddoc:generic="shared.frameworks.coldbox_3_00.system.ioc.config.Mapping">
    	<cfargument name="initArguments" 	required="false"	default="#structnew()#" 	hint="The constructor structure of arguments to passthrough when initializing the instance" colddoc:generic="struct"/>
		<cfscript>
    ...
    <cffunction name="autowire" output="false" access="public" returntype="any" hint="I wire up target objects with dependencies either by mappings or a-la-carte autowires">
		...
		<cfargument name="mapping" 				required="false" 	hint="The object mapping with all the necessary wiring metadata. Usually passed by scopes and not a-la-carte autowires" colddoc:generic="shared.frameworks.coldbox_3_00.system.ioc.config.Mapping"/>
    ...
    <cffunction name="setParent" output="false" access="public" returntype="void" hint="Link a parent Injector with this injector">
    	<cfargument name="injector" required="true" hint="A WireBox Injector to assign as a parent to this Injector" colddoc:generic="shared.frameworks.coldbox_3_00.system.ioc.Injector">
    	<cfset instance.parent = arguments.injector>
    </cffunction>
	
	<!--- getParent --->
    <cffunction name="getParent" output="false" access="public" returntype="any" hint="Get a reference to the parent injector instance, else an empty simple string meaning nothing is set" colddoc:generic="shared.frameworks.coldbox_3_00.system.ioc.Injector">
    	<cfreturn instance.parent>
    </cffunction>
	
	<!--- getObjectPopulator --->
    <cffunction name="getObjectPopulator" output="false" access="public" returntype="any" hint="Get an object populator useful for populating objects from JSON,XML, etc." colddoc:generic="shared.frameworks.coldbox_3_00.system.core.dynamic.BeanPopulator">
    	<cfreturn createObject("component","shared.frameworks.coldbox_3_00.system.core.dynamic.BeanPopulator").init()>
    </cffunction>
	
	<!--- getColdbox --->
    <cffunction name="getColdbox" output="false" access="public" returntype="any" hint="Get the instance of ColdBox linked in this Injector. Empty if using standalone version" colddoc:generic="shared.frameworks.coldbox_3_00.system.web.Controller">
	...
	<!--- getCacheBox --->
    <cffunction name="getCacheBox" output="false" access="public" returntype="any" hint="Get the instance of CacheBox linked in this Injector. Empty if using standalone version" colddoc:generic="shared.frameworks.coldbox_3_00.system.cache.CacheFactory">
	...
	<!--- getLogBox --->
    <cffunction name="getLogBox" output="false" access="public" returntype="any" hint="Get the instance of LogBox configured for this Injector" colddoc:generic="shared.frameworks.coldbox_3_00.system.logging.LogBox">
	...
	<cffunction name="getBinder" access="public" returntype="any" output="false" hint="Get the Injector's configuration binder object" colddoc:generic="shared.frameworks.coldbox_3_00.system.ioc.config.Binder">
    ...
    <cffunction name="removeFromScope" output="false" access="public" returntype="void" hint="Remove the Injector from scope registration if enabled, else does nothing">
    	<cfscript>
			...
			if( scopeInfo.enabled ){
				createObject("component","shared.frameworks.coldbox_3_00.system.core.collections.ScopeStorage")
					.init()
...
<!----------------------------------------- PRIVATE ------------------------------------->	
    ...
    <cffunction name="registerScopes" output="false" access="private" returntype="void" hint="Register all internal and configured WireBox Scopes">
    	<cfscript>
    		...
    		// register no_scope
			instance.scopes["NOSCOPE"] = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.scopes.NoScope").init( this );
			// register singleton
			instance.scopes["SINGLETON"] = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.scopes.Singleton").init( this );
			// is cachebox linked?
			if( isCacheBoxLinked() ){
				instance.scopes["CACHEBOX"] = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.scopes.CacheBox").init( this );
			}
			// CF Scopes and references
			instance.scopes["REQUEST"] 	= createObject("component","shared.frameworks.coldbox_3_00.system.ioc.scopes.CFScopes").init( this );
    ...
    <cffunction name="doScopeRegistration" output="false" access="private" returntype="void" hint="Register this injector on a user specified scope">
    	<cfscript>
			...
			createObject("component","shared.frameworks.coldbox_3_00.system.core.collections.ScopeStorage").init().put(scopeInfo.key, this, scopeInfo.scope);
    ...
    <cffunction name="configureLogBox" output="false" access="private" returntype="void" hint="Configure a standalone version of logBox for logging">
    	...
    	<cfscript>
			...
			config = createObject("component","shared.frameworks.coldbox_3_00.system.logging.config.LogBoxConfig").init(argumentCollection=args);
			
			// Create LogBox
			instance.logBox = createObject("component","shared.frameworks.coldbox_3_00.system.logging.LogBox").init( config );
			// Configure Logging for this injector
			instance.log = instance.logBox.getLogger( this );	
		</cfscript>
    </cffunction>
    ...
    <cffunction name="configureEventManager" output="false" access="private" returntype="void" hint="Configure a standalone version of a WireBox Event Manager">
    	<cfscript>
			...
			instance.eventManager = createObject("component","shared.frameworks.coldbox_3_00.system.core.events.EventPoolManager").init( instance.eventStates );
			// Debugging
			if( instance.log.canDebug() ){
	...
	<!--- Get ColdBox Util --->
	<cffunction name="getUtil" access="public" output="false" returntype="any" hint="Return the core util object" colddoc:generic="shared.frameworks.coldbox_3_00.system.core.util.Util">
    ...
    <cffunction name="buildBinder" output="false" access="private" returntype="any" hint="Load a configuration binder object according to passed in type">
		...
		<cfscript>
			...
			if( NOT isInstanceOf(arguments.binder, "shared.frameworks.coldbox_3_00.system.ioc.config.Binder") ){
				// simple data cfc, create native binder and decorate data CFC
				nativeBinder = createObject("component","shared.frameworks.coldbox_3_00.system.ioc.config.Binder").init(injector=this,config=arguments.binder,properties=arguments.properties);
			}
			else{
