...
/* ----------------------------------- CONSTRUCTOR ------------------------------ */
	...
	*/
	BaseORMService function init(string queryCacheRegion="ORMService.defaultCache",
		...
		ORMEventHandler = new shared.frameworks.coldbox_3_00.system.orm.hibernate.EventHandler();

		// Create our bean populator utility
		beanPopulator = createObject("component","shared.frameworks.coldbox_3_00.system.core.dynamic.BeanPopulator").init();
...
/* ----------------------------------- PUBLIC ------------------------------ */
	...
	*/
	any function createService(required string entityName,
		...
		return  CreateObject("component", "shared.frameworks.coldbox_3_00.system.orm.hibernate.VirtualEntityService").init(argumentCollection=arguments);
	}
