...
<cfcomponent extends="shared.frameworks.coldbox_3_00.system.Plugin"
...
<!---------------------------------------- PUBLIC FEED METHODS ------------------------------------------->
				...
				if(ListLen(arguments.feedURL) gt 1) {
					...
					if(i lte 1) {
						...
						for (j=1;j lte ArrayLen(feedCombArr[i].category);j=j+1) {
							...
							feed.category = createObject('component','shared.frameworks.coldbox_3_00.system.web.feeds.FeedReader').arrayOfStructsSort(feed.category,'tag');
						...
						if( arguments.itemsType eq "array" ) {
							...
							feed.items = createObject('component','shared.frameworks.coldbox_3_00.system.web.feeds.FeedReader').arrayOfStructsSort(feed.items,'datepublished');
						...
						// when arguments itemType is 'query', ...
							...
							feed.items = createObject('component','shared.frameworks.coldbox_3_00.system.web.feeds.FeedReader').querySortandTrim(feed.items,arguments.maxItems,'datepublished','desc');
						}
					}
	...
	<cffunction name="parseFeed" access="public" returntype="struct" hint="This parses a feed as a XML document and returns the results as a structure of elements" output="false">
		...
		<cfset var extras = createObject('component','shared.frameworks.coldbox_3_00.system.web.feeds.FeedReader').init(controller)>
