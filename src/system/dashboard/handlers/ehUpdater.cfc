...
<cfcomponent name="ehUpdater" extends="coldbox.system.eventhandler">
	...
	<cffunction name="dspUpdateSection" access="public" returntype="void">
		...
		<!--- Set the View --->
		<cfset Context.setView("vwUpdate")>
	</cffunction>
	...
	<cffunction name="dspUpdater" access="public" returntype="void">
		...
		<!--- Set the View --->
		<cfset Context.setView("update/vwUpdater")>
	</cffunction>
	...
	<cffunction name="docheckForUpdates" access="public" returntype="void">
		...
		<!--- Param Site--->
		<cfset Context.paramValue("distribution_site", "")>
		<cftry>
			<!--- Check for URL --->
			<cfif Context.getValue("distribution_site") eq "">
			...
			<!--- Get a WS Object --->
			<cfset updateWS = CreateObject("webservice",Context.getValue("distribution_site"))>
			<!--- Check for Updates --->
	...
	<cffunction name="dspUpdaterResults" access="public" returntype="void">
		...
		<!--- Set the View --->
		<cfset Context.setView("update/vwUpdaterResults")>
	</cffunction>
	...
	<cffunction name="doRollback" access="public" returntype="void">
		...
		<cfset getPlugin("messagebox").setMessage("error","There was an error while updating your system. Please look at the error below:<br><br>#URLDecode(Context.getValue("installerror"))#")>
	...
	<cffunction name="dspBackups" access="public" returntype="void">
		...
		<cfset Context.setValue("dirListing",dirListing)>
		<!--- grab if backup --->
		<cfif Context.getValue("finished","") eq "ok">
		...
		<cfset Context.setView("vwBackups")>
	</cffunction>
	
	<cffunction name="doRemoveFile" access="public" returntype="void">
		<!--- Get File info to remove --->
		<cfset var Filename = Context.getValue("filename")>
		<cfset var FilePath = Context.getValue("filepath")>
		<cftry>
			<!--- Remove File --->
	...
	<cffunction name="doDeliverFile" access="public" returntype="void">
		<cfset var DeliverFile = replace(urlDecode(Context.getValue("backupfile")),"{sep}",getSetting("OSFileSeparator",1))>
