...
<cfcomponent name="ehFileBrowser" extends="coldbox.system.eventhandler">
	...
	<cffunction name="dspBrowser" access="public" returntype="void">
		...
		<cfif valueExists("callbackItem")>
			<cfset session.callBackItem = Context.getValue("callbackItem")>
		...
		<cfset Context.paramValue("dir","/")>
		<cfif Context.getValue("dir") eq "">
			<cfset Context.setValue("dir","/")>
		</cfif>
		<!--- Init Options --->
		<cfif Context.getValue("dir") eq "/">
		...
		<cfelse>
			<cfset rc.currentRoot = Context.getValue("dir")>
		...
		<cfset Context.setView("tags/serverbrowser")>
	</cffunction>
	...
	<cffunction name="doNewFolder" access="public" returntype="void">
		...
		<cfif len(trim(Context.getValue("newFolder",""))) eq 0>
			<cfset getPlugin("messagebox").setMessage("warning", "Please enter a valid folder name.")>
		<cfelse>
		    <cfset newDir = Context.getValue("dir") & "/" & Context.getValue("NewFolder")>
		...
		<!--- Set the next event --->
		<cfset setNextEvent("ehFileBrowser.dspBrowser","dir=#Context.getValue("dir")#")>
	</cffunction>
