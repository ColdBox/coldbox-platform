<cfoutput>
<cfset qryDir = Context.getValue("qryDir")>
<cfset currentRoot = Context.getValue("currentRoot")>
<div id="FileBrowser">
...
<div style="width: 550px;margin: 10px; border:1px outset ##ddd;margin-left:auto;margin-right:auto;background-color: ##FFFFFF">
	...
	<div style="border-bottom:1px solid ##ddd; background-color: ##f5f5f5;padding: 5px;font-weight:bold;">
	   <a href="javascript:doEvent('#Context.getValue("xehBrowser")#', 'FileBrowser',{dir:'#jsstringFormat(currentroot)#'})" title="Refresh Listing"><img src="images/icons/arrow_refresh.png" align="absmiddle" border="0" title="Refresh Listing."></a>
	...
	<div style="line-height:20px;padding:3px;overflow:auto;height: 250px;font-weight:bold">
		...
		<cfif listlen(currentroot,"/") gte 1>
			<cfset tmpHREF = "javascript:doEvent('#Context.getValue("xehBrowser")#','FileBrowser',{dir:'#JSStringFormat(session.oldRoot)#'})">
		...
		<cfloop query="qryDir">
			<cfif qryDir.type eq "Dir">
				...
				<cfset tmpHREF = "javascript:doEvent('#Context.getValue("xehBrowser")#','FileBrowser',{dir:'#JSStringFormat(vURL)#'})">
	...
	<input type="button" id="createdir_btn" value="New Folder" style="font-size: 9xp" onClick="newFolder('#Context.getValue('xehNewFolder')#','#JSStringFormat(currentRoot)#')"> &nbsp;
