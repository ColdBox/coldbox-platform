...
Description :
	...
	The message is stored in the client scope. The look can be altered by
...
<cfcomponent name="messagebox" hint="This is the messagebox plugin. It uses the client scope to save messages. You will need the client scope to be available." extends="coldbox.system.plugin">
	...
	<cffunction name="getMessage" access="public" hint="Returns a structure of the message if it exists, else a blank structure." returntype="any" output="false">
		...
		<cfif structKeyExists(client,"fw_messagebox")>
			<cfwddx action="wddx2cfml" input="#client.fw_messagebox#" output="rtnStruct">
		<cfelse>
	...
	<!--- ************************************************************* --->
	<cffunction name="clearMessage" access="public" hint="Clears the message structure by deleting it from the client scope." output="false" returntype="void">
		<cfif structKeyExists(client,"fw_messagebox")>
			<cfset structdelete( client, "fw_messagebox")>
		</cfif>
	</cffunction>
	<!--- ************************************************************* --->
