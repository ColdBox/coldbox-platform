<cfcomponent extends="coldbox.system.testing.BaseTestCase">
    ...
    function testProcessMemento(){
        var data = {
            ...
            DIConstructorArgs: [ { name: "val", value: "0" }, { name: "transfer", ref: "transfer" } ],
            DIProperties: [
                { name: "user", dsl: "provider:User" },
                { name: "nonRequired", dsl: "provider:User", required: false }
            ],
            DISetters: [ { name: "Joke", value: "You!" }, { name: "service", ref: "MyService", argName: "MyService" } ],
            DIMethodArgs: [ { name: "Joke", value: "You!" }, { name: "service", ref: "MyService" } ]
        };
