...
<cfcomponent output="false" hint="The default WireBox Injector configuration object">
	...
	function configure( binder ){
		...
		wireBox = {
			...
			cacheBox : {
				enabled : false // configFile = "coldbox.system.ioc.config.CacheBox", An optional configuration file to use for loading CacheBox // cacheFactory = ""  A reference to an already instantiated CacheBox CacheFactory // classNamespace = "" A class path namespace to use to create CacheBox: Default=coldbox.system.cache or wirebox.system.cache
			},
