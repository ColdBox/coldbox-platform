component extends="coldbox.system.testing.BaseTestCase"{
	...
	function testbuildWebservice(){
		mapping = createMock( "coldbox.system.ioc.config.Mapping" ).init( "Buffer" );
		mapping.setPath( "http://localhost:8599/test-harness/remote/MyProxy.cfc?wsdl" );
	...
	function testBuildCFCWithArguments(){
		...
		mockInjector
			...
			.$( "containsInstance" ).$args( "myBean" ).$results( true )
			.$( "containsInstance" ).$args( "modelNotFound" ).$results( false );
	...
	function testBuildFactoryBean(){
		...
		// mocks
		mockTest = createStub();
		mockFactory = createStub().$( "getBean" ).$args(test="1",num2="3" ).$results(mockTest);
		mockInjector
			.$( "containsInstance", true )
			.$( "getInstance", mockFactory );
	...
	function testbuildSimpleDSL(){
		...
		// build it
		var r = builder.buildSimpleDSL( dsl="logbox:logger:test", targetID="unit", targetObject=mockStub );
	...
	function testBuildDependecyWithDSLDefaults(){
		...
		mockInjector.$( "containsInstance", true )
			.$( "getInstance", mockObject );
