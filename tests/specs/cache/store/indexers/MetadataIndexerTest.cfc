<cfcomponent extends="coldbox.system.testing.BaseModelTest">
	...
	function testGetSortedKeys(){
		md1 = {
			hits         : 40,
			created      : dateAdd( "n", -10, now() ),
			lastAccessed : dateAdd( "n", -7, now() )
		};
		index.setObjectMetadata( "md1", md1 );
		md2 = {
			hits         : 99,
			created      : dateAdd( "n", -1, now() ),
			lastAccessed : dateAdd( "n", -2, now() )
		};
		index.setObjectMetadata( "md2", md2 );
		md3 = {
			hits         : 1,
			created      : dateAdd( "n", -100, now() ),
			lastAccessed : dateAdd( "n", -83, now() )
		};
