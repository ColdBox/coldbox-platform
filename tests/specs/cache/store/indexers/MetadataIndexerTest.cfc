<cfcomponent extends="coldbox.system.testing.BaseModelTest">
    <cfscript>
    function setup(){
        index = createMock( "coldbox.system.cache.store.indexers.MetadataIndexer" ).init(
            "hits,timeout,created,lastAccessed"
        );
    }
    ...
    function getSetMetdataProperty(){
        tdate = now();
        md = { hits: randRange( 100, 10000 ), lastAccessed: tDate, created: tdate };
    ...
    function testGetSize(){
        ...
        var md = { hits: randRange( 100, 10000 ), lastAccessed: tDate, created: tdate };
        index.setObjectMetadata( "test", md );
        assertEquals( 1, index.getSize() );
    }

    function testGetSortedKeys(){
        md1 = { hits: 40, created: dateAdd( "n", -10, now() ), lastAccessed: dateAdd( "n", -7, now() ) };
        index.setObjectMetadata( "md1", md1 );
        md2 = { hits: 99, created: dateAdd( "n", -1, now() ), lastAccessed: dateAdd( "n", -2, now() ) };
        index.setObjectMetadata( "md2", md2 );
        md3 = { hits: 1, created: dateAdd( "n", -100, now() ), lastAccessed: dateAdd( "n", -83, now() ) };
