...
<cfset allCats = getvalue("allCats")>
	...
	<cfif not structKeyExists(form, "preview")>
		...
		<cfoutput>
		<script type="text/javascript" src="includes/relatedpostsjs.cfm?id=#URL.ID#"></script>
		</cfoutput>
		...
		<cfoutput>
		<form action="?event=ehAdmin.doSaveEntry&id=#url.id#" method="post" enctype="multipart/form-data" name="editForm">
		<table>
			<tr>
				<td align="right">title:</td>
				<td><input type="text" name="title" value="#htmlEditFormat(form.title)#" class="txtField" maxlength="100"></td>
			</tr>
			<tr valign="top">
			...
			<tr valign="top">
				...
				<td>
			        ...
			        <tr>
		    	      <td style="padding-right: 5px;"><select name="cboRelatedEntriesCats" multiple="multiple" size="4" onclick="doPopulateEntries(0); checkRememberEntries(); updateRememberEntries();" style="width:100%;"></select></td>
					...
					<cfif len(trim(form.cboRelatedEntries)) GT 0>
						<script type="text/javascript" src="includes/relatedpostsjs_error.cfm?c=#URLEncodedFormat(form.cboRelatedEntriesCats)#&p=#URLEncodedFormat(form.cboRelatedEntries)#"></script>
					</cfif>
			...
			<tr>
				...
				<cfif len(form.oldenclosure)>#listLast(form.oldenclosure,"/\")# <input type="submit" name="delete_enclosure" value="#getResource("deleteenclosure")#"></cfif>
			...
			<tr>
				<td>&nbsp;</td>
				<td><input type="submit" name="cancel" value="Cancel"> <input type="submit" name="preview" value="Preview"> <input type="submit" name="save" value="Save"></td>
			</tr>
		</table>
		</form>
		</cfoutput>
