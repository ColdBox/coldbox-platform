...
<cfset message = getvalue("message")>
<cfset entry = getvalue("entry")>
<cfset allCats = getvalue("allCats")>

<cfif not structKeyExists(form, "preview")>

	<cfoutput>
	<script type="text/javascript" src="includes/relatedpostsjs.cfm?id=#getValue("ID")#"></script>
	</cfoutput>
	...
	<cfoutput>
	<form action="?event=#getValue("xehSave")#&id=#getvalue("id")#" method="post" enctype="multipart/form-data" name="editForm">
	<table>
		<tr>
			<td align="right">title:</td>
			<td><input type="text" name="title" value="#htmlEditFormat(getValue("title"))#" class="txtField" maxlength="100"></td>
		</tr>
		<tr valign="top">
			<td align="right">body:</td>
			<td><textarea name="body" class="txtArea">#getValue("body")#</textarea></td>
		</tr>
		<tr>
			<td align="right">posted:</td>
			<td><input type="text" name="posted" value="#getValue("posted")#" class="txtField" maxlength="100"></td>
		</tr>
		<tr valign="top">
			...
			<select name="categories" multiple size=4 class="txtDropdown">
				...
				<option value="#categoryID#" <cfif valueExists("categories") and listFind(getValue("categories"),categoryID)>selected</cfif>>#categoryName#</option>
				</cfloop>
			</select><br>
			</cfif>
			<input type="text" name="newcategory" value="#htmlEditFormat(getValue("newcategory"))#" class="txtField" maxlength="50"> New Category</td>
		</tr>
		...
		<tr valign="top">
			...
			<td>
				...
				<cfif len(trim(getValue("cboRelatedEntries"))) GT 0>
					<script type="text/javascript" src="includes/relatedpostsjs_error.cfm?c=#URLEncodedFormat(getValue("cboRelatedEntriesCats"))#&p=#URLEncodedFormat(getValue("cboRelatedEntries"))#"></script>
				</cfif>
		...
		<tr>
			...
			<td><input type="text" name="alias" value="#getValue("alias")#" class="txtField" maxlength="100"></td>
		</tr>
		<tr>
			<td align="right">allow comments:</td>
			<td>
			<select name="allowcomments">
				<option value="true" <cfif getValue("allowcomments") is "true">selected</cfif>>Yes</option>
				<option value="false" <cfif getValue("allowcomments") is "false">selected</cfif>>No</option>
			</select>
			</td>
		</tr>
		<tr>
			...
			<td>
			<input type="hidden" name="oldenclosure" value="#getValue("oldenclosure")#">
			<input type="hidden" name="oldfilesize" value="#getValue("oldfilesize")#">
			<input type="hidden" name="oldmimetype" value="#getValue("oldmimetype")#">
			<cfif len(getValue("oldenclosure"))>#listLast(getValue("oldenclosure"),"/\")# <input type="submit" name="delete_enclosure" value="#getResource("deleteenclosure")#"></cfif>
		...
		<tr>
			...
			<option value="true" <cfif getValue("released") is "true">selected</cfif>>Yes</option>
			<option value="false" <cfif getValue("released") is "false">selected</cfif>>No</option>
			</select>
			</td>
		</tr>
		<tr>
...
<cfelse>
	...
	#application.blog.renderEntry(getValue("body"),false,getValue("oldenclosure"))#
	</div>
	</cfoutput>
