...
<cfset user = Context.getValue("user")>
<cfset subs = Context.getValue("subs")>
...
<table width="500" cellpadding="6" class="tableDisplay" cellspacing="1" border="0">
	...
	<tr class="tableRowMain">
		...
		<cfif Context.getValue("confirm","") eq "profile">
	...
	<tr class="tableRowMain">
		...
		<input type="hidden" name="event" value="#Context.getValue("xehSaveProfile")#">
		<table>
			<tr>
				<td><b>Username:</b></td>
				<td>#user.username#</td>
...
<table width="500" cellpadding="6" class="tableDisplay" cellspacing="1" border="0">
	...
	<cfif Context.getValue("confirm","") eq "subscribe">
	<tr class="tableRowMain">
		<td>
		#getPlugin("messagebox").renderit()#
		</td>
			...
			<cfloop query="subs">
				<tr>
					...
					<cfif len(conferenceidfk)>
						...
						<cfset link = "index.cfm?event=#Context.getValue("xehForums")#&conferenceid=#conferenceidfk#">
					<cfelseif len(forumidfk)>
						<cfset data = application.forum.getForum(forumidfk)>
						<cfset label = "Forum">
						<cfset link = "index.cfm?event=#Context.getValue("xehThreads")#&forumid=#threadidfk#">
					<cfelse>
						<cfset data = application.thread.getThread(threadidfk)>
						<cfset label = "Thread">
						<cfset link = "index.cfm?event=#Context.getValue("xehMessages")#&threadid=#threadidfk#">
					</cfif>
					#label#:
					</td>
					<td><a href="#link#">#data.name#</a></td> 
					<td>[<a href="index.cfm?event=#Context.getValue("xehRemoveSub")#&removeSub=#id#">Unsubscribe</a>]</td>
			</cfloop>
			</table>
			</p>
		</cfif>
