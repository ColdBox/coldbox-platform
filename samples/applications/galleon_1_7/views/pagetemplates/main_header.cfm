...
<head>
	...
	<meta name="description" content="#requestContext.getValue("title")#">
	<meta name="keywords" content="#replace(requestContext.getValue("title")," : ", ",","all")#"> 
...
<table width="750" cellpadding="0" cellspacing="4" border="0">
	<tr>
		...
		<a href="index.cfm?event=#requestContext.getValue("xehProfile")#">Profile</a> | 
		<a href="index.cfm?event=#requestContext.getValue("xehSearch")#">Search</a> | 
		<cfset thisPage = cgi.script_name & "?" & reReplace(cgi.query_string,"logout=1","")>
		
		<cfif not requestContext.valueExists("ref")>
			<cfset link = "?event=#requestContext.getValue("xehLogin")#&ref=#urlEncodedFormat(thisPage)#">
		<cfelse>
			<cfset link = "?event=#requestContext.getValue("xehLogin")#&ref=#urlEncodedFormat(ref)#">
		</cfif>	
		<cfif isLoggedOn()><a href="index.cfm?logout=1">Logout</a><cfelse><a href="#link#">Login</a></cfif>
		<cfif isDefined("request.conference")> | <a href="index.cfm?event=#requestContext.getValue("xehRSS")#&conferenceid=#request.conference.id#">RSS</a></cfif>
		</span>
		</td>
	</tr>
	<tr bgcolor="##235577">
