...
<table width="500" cellpadding="6" class="tableDisplay" cellspacing="1" border="0">
	...
	<tr class="tableRowMain">
		...
		<input type="hidden" name="event" value="ehForums.doSaveProfile">
		<table>
			<tr>
				<td><b>Username:</b></td>
				<td>#user.username#</td>
			...
			<cfloop query="subs">
				<tr>
					...
					<cfif len(conferenceidfk)>
						...
						<cfset link = "index.cfm?event=ehForums.dspforums&conferenceid=#conferenceidfk#">
					<cfelseif len(forumidfk)>
						<cfset data = application.forum.getForum(forumidfk)>
						<cfset label = "Forum">
						<cfset link = "index.cfm?event=ehForums.dspThreads&forumid=#threadidfk#">
					<cfelse>
						<cfset data = application.thread.getThread(threadidfk)>
						<cfset label = "Thread">
						<cfset link = "index.cfm?event=ehForums.dspMessages&threadid=#threadidfk#">
					</cfif>
					#label#:
					</td>
					<td><a href="#link#">#data.name#</a></td> 
					<td>[<a href="index.cfm?event=ehForums.doRemoveSub&removeSub=#id#">Unsubscribe</a>]</td>
			</cfloop>
			</table>
			</p>
		</cfif>
