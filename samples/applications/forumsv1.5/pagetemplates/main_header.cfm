...
<table width="750" cellpadding="0" cellspacing="4" border="0">
	<tr>
		...
		<a href="index.cfm?event=ehForums.dspProfile">Profile</a> | 
		<a href="index.cfm?event=ehForums.dspSearch">Search</a> | 
		<cfset thisPage = cgi.script_name & "?" & reReplace(cgi.query_string,"logout=1","")>
		
		<cfif not valueExists("ref")>
			<cfset link = "?event=ehForums.dspLogin&ref=#urlEncodedFormat(thisPage)#">
		<cfelse>
			<cfset link = "?event=ehForums.dspLogin&ref=#urlEncodedFormat(ref)#">
		</cfif>	
		<cfif request.udf.isLoggedOn()><a href="index.cfm?logout=1">Logout</a><cfelse><a href="#link#">Login</a></cfif>
		<cfif isDefined("request.conference")> | <a href="index.cfm?event=ehForums.dspRss&conferenceid=#request.conference.id#">RSS</a></cfif>
		</span>
		</td>
	</tr>
	<tr bgcolor="##235577">
