...
<cfscript>
	...
	function testbuildcfc(){
		...
		mapping.setPath("coldbox.test.testmodel.ioc.Simple");
	...
	function testBuildCFCWithArguments(){
		//mocks
		mockObject = getMockBox().createMock("coldbox.test.testmodel.ioc.Simple");
		...
		mapping = getMockBox().createMock("coldbox.system.ioc.config.Mapping").init("MyCFC");
		mapping.setPath("coldbox.test.testmodel.ioc.SimpleConstructors")
	...
	function testregisterCustomBuilders(){
		customDSL = {
			coolLuis = "coldbox.test.specs.ioc.dsl.MyTestingDSL"
		};
