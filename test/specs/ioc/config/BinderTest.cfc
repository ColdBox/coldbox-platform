...
<cfscript>
	...
	function setup(){
		...
		dataConfigPath = "coldbox.test.specs.ioc.config.samples.SampleWireBox";
	...
	function testMapDirectory(){
		config.mapDirectory("coldbox.test.testModel");
		assertTrue( structCount(config.getMappings()) gte 18 );

		config.reset();

		config.mapDirectory(packagePath="coldbox.test.testModel",include="ioc.*");
		assertTrue( structCount(config.getMappings) gte 10 );

		config.reset();

		config.mapDirectory(packagePath="coldbox.test.testModel",exclude="ioc.*");
		//debug( config.getMappings() );
		assertTrue( structCount(config.getMappings()) gt 5 );
		
		// with influence
		config.reset();
		config.mapDirectory(packagePath="coldbox.test.testModel.ioc", influence=influenceUDF);
		assertEquals( "singleton", config.getMapping("Simple").getScope() );
		
		// with filters
		config.reset();
		config.mapDirectory(packagePath="coldbox.test.testModel.ioc", filter=filterUDF);
