...
<cfscript>
	...
	function testbuildBinder(){
		...
		binder = injector.buildBinder("coldbox.test.specs.ioc.config.samples.SampleWireBox",{});
		assertTrue( isObject(binder) );

		//2: WireBox Binder instance
		binder = getMockBox().createMock("coldbox.test.specs.ioc.config.samples.WireBox");
		binder.$("configure");
		injector.buildBinder(binder,{});
		assertEquals(1, binder.$count("configure") );

		//3: Simple Binder CFC
		binder = getMockBox().createMock("coldbox.test.specs.ioc.config.samples.SampleWireBox");
	...
	function testRegisterListeners(){
		...
		//Mocking
		listeners = [
			{class="coldbox.test.specs.ioc.config.listeners.MyListener",
		...
		// exception
		listeners = [
			{class="coldbox.test.specs.ioc.config.listeners.MyLister",
			 name="myDude", properties={}}
		];
		getMockBox().prepareMock( binder ).$("getListeners", listeners);
		try{
