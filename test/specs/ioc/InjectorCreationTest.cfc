...
<cfscript>
	...
	function setup(){
		// init with defaults
		injector = getMockBox().createMock("coldbox.system.ioc.Injector").init("coldbox.test.specs.ioc.config.samples.InjectorCreationTestsBinder");
	...
	function testLocateInstance(){
		...
		assertEquals("coldbox.test.testmodel.ioc.category.CategoryBean", r);
