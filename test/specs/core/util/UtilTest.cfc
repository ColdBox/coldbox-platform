...
<cfscript>
	function setup(){
		...
		class1 = CreateObject("component","coldbox.test.specs.core.util.class1");
	}

	function isInstanceCheck(){
		test = createObject("component","coldbox.test.testhandlers.BaseTest");
		assertTrue( util.isInstanceCheck( test, "coldbox.system.EventHandler") );

		test = createObject("component","coldbox.test.testhandlers.ehTest");
		assertTrue( util.isInstanceCheck( test, "coldbox.system.EventHandler") );

		test = createObject("component","coldbox.test.testhandlers.TestNoInheritance");
	...
	function testGetInheritedMetaData(){
		...
		md = util.getInheritedMetaData("coldbox.test.specs.core.util.class1");
		testGetInheritedMetaDataHelper(md);

	}

	function testGetInheritedMetaDataStopRecursion(){
		stopRecursions = ["coldbox.test.specs.core.util.class2"];

		md = util.getInheritedMetaData(class1,stopRecursions);
		testGetInheritedMetaDataStopRecursionHelper(md);

		md = util.getInheritedMetaData("coldbox.test.specs.core.util.class1",stopRecursions);
		testGetInheritedMetaDataStopRecursionHelper(md);
	...
	private function testGetInheritedMetaDataHelper(md){
		...
		assertEquals( md.inheritanceTrail[1], "coldbox.test.specs.core.util.class1" );
		assertEquals( md.inheritanceTrail[2], "coldbox.test.specs.core.util.class2" );
		assertEquals( md.inheritanceTrail[3], "coldbox.test.specs.core.util.class3" );
