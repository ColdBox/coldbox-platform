    ...
    <cffunction name="buildProviderMixer" output="false" access="public" returntype="any" hint="Used to provider providers via mixers on targeted objects">
    	<cfscript>
			...
			return this.$wbScopeStorage.get(this.$wbScopeInfo.key, this.$wbScopeInfo.scope)
						.getInstance( name=getMetaData(this).name, dsl=this.$wbProviders[ getFunctionCalledName() ], targetObject=this);
		</cfscript>
    </cffunction>
...
<!------------------------------------------- DSL BUILDER METHODS ------------------------------------------>
	...
	<cffunction name="getProviderDSL" access="private" returntype="any" hint="Get dependencies using the our provider pattern DSL" output="false" >
		...
		<cfscript>
			...
			provider = createObject("component","coldbox.system.ioc.Provider").init(instance.injector.getScopeRegistration(), instance.injector.getScopeStorage(), providerName, providerDSL);
			if(structKeyExists(arguments,"targetObject") AND isObject(arguments.targetObject)){
				provider.$targetObject = arguments.targetObject;
			}
			return provider;
		</cfscript>
	</cffunction>
