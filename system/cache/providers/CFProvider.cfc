...
component serializable="false" implements="coldbox.system.cache.ICacheProvider"{
    ...
    void function configure() output=false{
		...
		lock name="CFProvider.config.#instance.cacheID#" type="exclusive" throwontimeout="true" timeout="20"{
			...
			if ( thisCacheName == "object") {
				...
				// In ColdFusion 10, we need to make sure the cache region exists before we can use it.
				if( val(listFirst(Server.ColdFusion.ProductVersion)) gte 10 ){
					if( !cacheRegionExists(thisCacheName) ){
						cacheRegionNew(thisCacheName);
					}
				}
    ...
    */
    ...
    if ( thisCacheName == "object") {
			...
			return cacheGetMetadata( arguments.objectKey, 'object', thisCacheName );
		}
	}
