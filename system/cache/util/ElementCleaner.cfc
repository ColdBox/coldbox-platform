...
<cfcomponent output="false" hint="A ColdBox utility to help clean cached objects for ColdBox Application Caches">
    ...
    <cffunction name="init" access="public" output="false" returntype="ElementCleaner" hint="Constructor">
        <cfargument
            ...
            type="any"
            required="true"
            hint="The associated cache manager/provider of type: coldbox.system.cache.providers.ICacheProvider"
    ...
    <cffunction
        name="getAssociatedCache"
        access="public"
        output="false"
        returntype="any"
        hint="Get the associated cache provider/manager of type: coldbox.system.cache.providers.ICacheProvider"
        doc_generic="coldbox.system.cache.providers.ICacheProvider"
    >
    ...
    <cffunction
        name="clearByKeySnippet"
        access="public"
        returntype="void"
        hint="Clears keys using the passed in object key snippet"
        output="false"
    >
        <!--- ************************************************************* --->
    ...
    >
        ...
        var cacheKey = getAssociatedCache().getEventCacheKeyPrefix() & replace(
        ...
        if( len( arguments.queryString ) neq 0 ){
            cacheKey &= "-" & getAssociatedCache()
                .getEventURLFacade()
                .buildHash( arguments.queryString );
        }
