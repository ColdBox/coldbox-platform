...
<cfcomponent
	name="ColdboxProxy"
	output="false"
	hint="This component is the coldbox remote proxy used for remote operations."
>
	<cfscript>
	...
	<cffunction
		...
		output="false"
		access="private"
		hint="Autowire the proxy on creation. This references the super class only, we use cgi information to get the actual proxy component path."
	>
		<cfscript>
	...
	<!--- getRemotingUtil --->
	<cffunction
		name="getRemotingUtil"
		output="false"
		access="private"
		returntype="coldbox.system.remote.RemotingUtil"
		hint="Get a reference to the ColdBox Remoting utility class."
	>
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="Process a remote call into ColdBox's event model and return data/objects back. If no results where found, this method returns null/void"
	>
	...
	<cffunction
		name="announceInterception"
		output="false"
		access="private"
		returntype="boolean"
		hint="Process a remote interception"
	>
		<!--- ************************************************************* --->
		...
		<cfargument
			...
			type="any"
			required="false"
			hint="This intercept data structure to announce with"
		/>
		<!--- ************************************************************* --->

		<cfset var cbController = getController()>
		<cftry>
			<cfif NOT structKeyExists( arguments, "interceptData" )>
				<cfset arguments.interceptData = structNew()>
			</cfif>
			<cfset cbController
	...
	<cffunction
		...
		access="private"
		returntype="void"
		hint="Handle a ColdBox Proxy Exception"
	>
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="Get the controller from application scope."
	>
		<cfscript>
		// Verify ColdBox
		verifyColdBox();
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="Get the CacheBox reference."
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="Get the WireBox Injector reference of this application."
	>
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="Locates, Creates, Injects and Configures an object model instance"
	>
		<cfargument name="name" required="true" hint="The mapping name or CFC instance path to try to build up"/>
		<cfargument
			name="dsl"
			required="false"
			hint="The dsl string to use to retrieve the instance model object, mutually exclusive with 'name'"
		/>
		<cfargument
			name="initArguments"
			required="false"
			hint="The constructor structure of arguments to passthrough when initializing the instance"
			doc_generic="struct"
		/>
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="Get the LogBox reference of this application."
	>
	...
	<cffunction
		...
		output="false"
		access="private"
		returntype="coldbox.system.logging.Logger"
		hint="Get the root logger reference."
	>
	...
	<cffunction
		...
		access="private"
		returntype="coldbox.system.logging.Logger"
		hint="Get a named logger reference."
	>
		<cfargument
			name="category"
			type="any"
			required="true"
			hint="The category name to use in this logger or pass in the target object will log from and we will inspect the object and use its metadata name."
		/>
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="DEPRECATED: Plugin factory, returns a new or cached instance of a plugin."
		output="false"
	>
	...
	<cffunction name="getInterceptor" access="private" output="false" returntype="any" hint="Get an interceptor">
		...
		<cfargument
			...
			required="false"
			type="string"
			hint="The name of the interceptor to search for"
		/>
		<cfargument
			name="deepSearch"
			required="false"
			type="boolean"
			default="false"
			hint="By default we search the cache for the interceptor reference. If true, we search all the registered interceptor states for a match."
		/>
		<!--- ************************************************************* --->
		<cfreturn getController()
			.getInterceptorService()
	...
	<cffunction
		...
		access="private"
		returntype="any"
		hint="Create or retrieve model objects by convention"
		output="false"
	>
		<!--- ************************************************************* --->
	...
	<cffunction
		...
		output="false"
		returntype="any"
		hint="Get a CacheBox Cache Provider"
		doc_generic="coldbox.system.cache.providers.IColdBoxProvider"
	>
	...
	<cffunction
		...
		output="false"
		returntype="void"
		hint="Load a coldbox application, and place the coldbox controller in application scope for usage. If the application is already running, then it will not re-do it, unless you specify the reload argument or the application expired."
	>
		<!--- ************************************************************* --->
		<cfargument
			name="appMapping"
			type="string"
			required="true"
			hint="The absolute location of the root of the coldbox application. This is usually where the Application.cfc is and where the conventions are read from."
		/>
		<cfargument
			name="configLocation"
			type="string"
			required="false"
			default=""
			hint="The absolute location of the config file to override, if not passed, it will try to locate it by convention."
		/>
		<cfargument
			name="reloadApp"
			type="boolean"
			required="false"
			default="false"
			hint="Flag to reload the application or not"
		/>
		<cfargument
			name="appKey"
			type="string"
			required="true"
			default="#COLDBOX_APP_KEY#"
			hint="The application key name to use, defaults to 'cbController'"
		/>
		<!--- ************************************************************* --->
		<cfset var cbController = "">
		...
		<cfif not structKeyExists( application, arguments.appKey ) or not application[ arguments.appKey ].getColdboxInitiated() or arguments.reloadApp>
			<cflock type="exclusive" name="#appHash#" timeout="30" throwontimeout="true">
				<cfscript>
				if (
					not structKeyExists( application, arguments.appKey ) OR NOT application[ arguments.appKey ].getColdboxInitiated() OR
					arguments.reloadApp
				) {
					// Cleanup, Just in Case
					structDelete( application, arguments.appKey );
					// Load it Up baby!!
					cbController = createObject( "component", "coldbox.system.web.Controller" ).init(
	...
	<cffunction
		...
		access="private"
		output="false"
		returntype="coldbox.system.core.util.Util"
	...
	<cffunction
		name="setCOLDBOX_APP_KEY"
		access="private"
		returntype="void"
		output="false"
		hint="Override the name of the coldbox application key used."
	>
