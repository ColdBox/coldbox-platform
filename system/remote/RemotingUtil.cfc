 ...
 * A cool remoting utililty component
 */
component output="false" hint="A cool remoting utililty component" {
	/*  
	...
	*/
	 ...
	 */
	public void function clearHeaderBuffer() output=false {
		var my = structnew();
		switch ( trim( server.coldfusion.productname ) ) {
			case  "ColdFusion Server":
				...
				my.method = my.out.getClass().getDeclaredMethod( "initHeaderBuffer", arrayNew(1) );
				my.method.setAccessible(true);
				my.method.invoke(my.out, arrayNew(1));
				break;
			case  "Lucee":
				my.out = getPageContext().getOut();
