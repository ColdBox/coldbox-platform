...
component extends="coldbox.system.testing.BaseTestCase" {
	...
	function setup(){
		...
		if ( NOT structKeyExists( md, "handler" ) ) {
			throw(
				"handler annotation not found on component tag",
				"Please declare a 'handler=path' annotation",
				"BaseHandlerTest.InvalidStateException"
			);
		}
		...
		if ( NOT getUtil().isFamilyType( "handler", variables.handler ) ) {
			...
			if ( structKeyExists( variables.handler, "$cbInit" ) ) {
				variables.handler.$cbInit( mockController );
			}
		}
	}
