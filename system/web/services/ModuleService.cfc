...
component extends="coldbox.system.web.services.BaseService" {
	...
	boolean function registerModule(
		...
		lock name="module#getController().getAppHash()#.registration.#arguments.modulename#"
			...
			variables.interceptorService.processState(
				...
				{ moduleConfig : mConfig, moduleName : arguments.moduleName }
			);
	...
	ModuleService function activateModule( required moduleName ){
		...
		mConfig.dependencies.each( function(thisDependency) {
			if ( variables.logger.canInfo() ) {
				variables.logger.info(
					"Activating #moduleName# requests dependency activation: #thisDependency#"
				);
			}
		...
		if ( modules[ arguments.moduleName ].activated ) {
			...
			if ( variables.logger.canInfo() ) {
				variables.logger.info(
					"Module #arguments.moduleName# already activated during dependecy activation, skipping activation."
				);
			}
			return this;
		}
		...
		lock name="module#getController().getAppHash()#.activation.#arguments.moduleName#"
			...
			variables.interceptorService.processState(
				"preModuleLoad",
				{ moduleLocation : mConfig.path, moduleName : arguments.moduleName }
			);
			...
			if ( mConfig.autoMapModels AND directoryExists( mconfig.modelsPhysicalPath ) ) {
				...
				if ( fileExists( mconfig.modelsPhysicalPath & "/#arguments.moduleName#.cfc" ) ) {
					binder
						.map( [ "@#arguments.moduleName#", "@#mConfig.modelNamespace#" ] )
			...
			variables.interceptorService.processState(
				"postModuleLoad",
				{ moduleLocation : mConfig.path, moduleName : arguments.moduleName, moduleConfig : mConfig }
			);
		...
		lock name="module#getController().getAppHash()#.unload.#arguments.moduleName#"
			...
			if ( structKeyExists( variables.mConfigCache[ arguments.moduleName ], "onUnload" ) ) {
				try{
					...
					variables.logger.error(
						"Error unloading module: #arguments.moduleName#. #e.message# #e.detail#",
						e
					);
			...
			for ( var x = 1; x lte arrayLen( appConfig.modules[ arguments.moduleName ].interceptors ); x++ ) {
				variables.interceptorService.unregister(
					appConfig.modules[ arguments.moduleName ].interceptors[ x ].name
				);
			}
	...
	function loadModuleConfiguration( required struct config, required moduleName ){
		...
		if ( mConfig.parseParentSettings ) {
			...
			structAppend( mConfig.settings, parentModuleSettings[ mConfig.modelNamespace ], true );
		}
