...
component extends="coldbox.system.web.services.BaseService" accessors="true" {
    ...
    function onMissingMethod( missingMethodName, missingMethodArguments = {} ){
        return invoke( variables.router, arguments.missingMethodName, arguments.missingMethodArguments );
    }
    ...
    private any function renderResponse( required route, required event ){
        ...
        // render it out
        event.renderdata( data = theResponse, statusCode = aRoute.statusCode, statusText = aRoute.statusText ).noExecution();
    }
    ...
    private function getCleanedPaths( required rc, required event ){
        ...
        results[ "scriptName" ] = trim(
            reReplaceNoCase( getCGIElement( "script_name", arguments.event ), "[/\\]index\.cfm", "" )
        );
        results[ "domain" ] = trim(
            reReplace(
