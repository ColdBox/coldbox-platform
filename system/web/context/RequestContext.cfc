...
component serializable=false accessors="true" {
    ...
    function collectionAppend( required struct collection, boolean overwrite = false, boolean private = false ){
        if( arguments.private ){
            structAppend( variables.privateContext, arguments.collection, arguments.overwrite );
        }else{
            structAppend( variables.context, arguments.collection, arguments.overwrite );
        }
        return this;
    }
    ...
    string function getHTMLBaseURL(){
        return reReplaceNoCase( buildLink( to = "", ssl = isSSL() ), "index.cfm\/?", "" ) & getContextRoot().reReplace(
            "^/",
            ""
        );
    }
    ...
    string function route( required name, struct params = {}, boolean ssl ){
        ...
        if( arrayLen( foundRoute ) ){
            var args = { to: entryPoint & foundRoute[ 1 ].pattern, ssl: javacast( "null", "" ) };
        ...
        if( variables.SESEnabled ){
            ...
            if( arguments.translate ){
                ...
                if( len( arguments.queryString ) ){
                    ...
                    arguments.to = arguments.to & replace(
    ...
    private function renderWithFormats(){
        ...
        if( arrayFindNoCase( arguments.formats, variables.context.format ) ){
            ...
            switch( variables.context.format ){
                ...
                case "pdf": {
                    arguments.type = "pdf";
                    arguments.data = variables.controller
                        .getRenderer()
                        .renderView( view = viewToRender );
                    return renderData( argumentCollection = arguments );
                }
                case "html":
                case "plain": {
