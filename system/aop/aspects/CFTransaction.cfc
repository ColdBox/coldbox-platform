	...
	methodMatcher="annotatedWith:transactional"
{
	...
	function invokeMethod( required invocation ) output="false"{
		...
		try{
			...
			log.error(
				"An exception ocurred in the AOPed transactio for target: #arguments.invocation.getTargetName()#, method: #arguments.invocation.getMethod()#: #cfcatch.message# #cfcatch.detail#",
				cfcatch
			);
			rethrow;
		}
