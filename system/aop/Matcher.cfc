/*-----------------------------------------------------------------------
********************************************************************************
Copyright Since 2005 ColdBox Framework by Luis Majano and Ortus Solutions, Corp
www.ortussolutions.com
********************************************************************************

Author 	    :	original: Luis Majano, cfscript: Ben Koshy
Description :
	I match class and method names to data in this matcher
-----------------------------------------------------------------------*/
component
{
	hint   = "I match class and method names to data in this matcher"
	output = "false"

	/*
	* @hint		Constructor
	* @output 	false
	*/
	public any function init(){
		reset();

		return this;
	} // init()

	/*
	* @hint		Reset the matcher memento to defaults
	* @output 	false
	*/
	public any function reset(){

		// prepare instance for this matcher
		instance = {
			any = false,
			returns = "",
			annotation = "",
			mappings = "",
			instanceOf = "",
			regex = "",
			methods = ""
		};
		
		// Aggregators
		instance.and = "";
		instance.or  = "";	
		
		return this;	
	} // reset()

	/*
	* @hint		Get the matcher memento
	* @output 	false
	*/
	public any function getMemento(){
		return instance;
	} // getMemento()

	/*
	* @hint		Matches a class to this matcher according to its criteria
	* @output 	false
	*
	* @target.hint				The target to match against to
	* @mapping.hint				The target mapping to match against to
	* @mapping.colddoc:generic	coldbox.system.ioc.config.Mapping
	*/
	public boolean function matchClass(
		required any target,
		required any mapping
	){
		var results = matchClassRules(argumentCollection=arguments);
		
		// AND matcher set?
		if( isObject( instance.and ) ){ return ( results AND instance.and.matchClass( argumentCollection=arguments )); }
		// OR matcher set?
		if( isObject( instance.or ) ){ return ( results OR instance.or.matchClass( argumentCollection=arguments )); }
		
		return results;
	} // matchClass()

	/*
	* @hint		Matches a method to this matcher according to its criteria
	* @output 	false
	*
	* @metadata.hint	The UDF metadata to use for matching
	*/
	public boolean function matchMethod( required any metadata ){

		var results = matchMethodRules(arguments.metadata);
		
		// AND matcher set?
		if( isObject( instance.and ) ){ return (results AND instance.and.matchMethod(arguments.metadata) ); }
		// OR matcher set?
		if( isObject( instance.or ) ){ return (results OR instance.or.matchMethod(arguments.metadata) ); }
		
		return results;			
	} // matchMethod()

	/*
	* @hint		Match against any method name or class path
	* @output 	false
	*/
	public any function any(){
		instance.any = true;
		return this;
	} // any()

	/*
	* @hint		Match against return types in methods only
	* @output 	false
	*
	* @type.hint	The type of return to match against.  Only for method matching
	*/
	public any function returns( required any type ){
		instance.returns = arguments.type;
		return this;
	} // returns()

    /*
	* @hint		Matches annotations on components or methods with or without a value
	* @output 	false
	*
	* @annotation.hint	The annotation to discover
	* @value.hint		The value of the annotation that must match. OPTIONAL
	*/
	public any function annotatedWith(
		required any annotation,
		any value
	){
		instance.annotation = arguments.annotation;
		// the value of the annotation
		if( structKeyExists( arguments, "value" )){
			instance.annotationValue = arguments.value;
		}
		return this;
	} // annotatedWith()

    /*
	* @hint		Match one, list or array of mapping names. Class Matching Only.
	* @output 	false
	*
	* @mappings.hint	One, list or array of mappings to match
	*/
	public any function mappings(
		required any mappings,
	){
		if( isArray( arguments.mappings )){ arguments.mappings = arrayToList( arguments.mappings );}
		instance.mappings = arguments.mappings;
		return this;
	} // mappings()

    /*
	* @hint		Matches against a family of components according to the passed classPath. Class Matching Only.
	* @output 	false
	*
	* @classPath.hint	The class path to verify instance of
	*/
	public any function instanceOf(
		required any classPath,
	){
		instance.instanceOf = arguments.classPath;
		return this;
	} // instanceOf()

    /*
	* @hint		Matches a class path or method name to this regular expression
	* @output 	false
	*
	* @regex.hint	The regular expression to match against
	*/
	public any function regex(
		required any regex,
	){
		instance.regex = arguments.regex;
		return this;	    
	} // regex()

    /*
	* @hint		A list, one or an array of methods to explicitly match
	* @output 	false
	*
	* @methods.hint	One, list or array of methods to match
	*/
	public any function methods(
		required any methods,
	){
		if( isArray( arguments.methods )){ arguments.methods = arrayToList( arguments.methods );}
		instance.methods = arguments.methods;
		return this;
	} // methods()

    /*
	* @hint		AND this matcher with another matcher
	* @output 	false
	*
	* @matcher.hint				The matcher to AND this matcher with
	* @matcher.colddoc:generic	coldbox.system.aop.Matcher
	*/
	public any function andMatch(
		required any matcher,
	){
		instance.and = arguments.matcher;
		return this;
	} // andMatch()

    /*
	* @hint		OR this matcher with another matcher
	* @output 	false
	*
	* @matcher.hint				The matcher to OR this matcher with
	* @matcher.colddoc:generic	coldbox.system.aop.Matcher
	*/
	public any function orMatch(
		required any matcher,
	){
		instance.or = arguments.matcher;
		return this;
	} // orMatch()
	
    /*
    * ****************************** PRIVATE ******************************
    */

	/*
	* @hint		Go through all the rules in this matcher and match
	* @output 	false
	*
	* @metadata.hint	The UDF metadata to use for matching
	*/
	private boolean function matchMethodRules( required any metadata ){
		// Some metadata defaults
		var name 	= arguments.metadata.name;
		var returns = "any";
		
		if( structKeyExists(arguments.metadata, "returntype") ){ returns = arguments.metadata.returntype; }
		
		// Start with any()
		if( instance.any ){ return true; }
		// Check explicit methods
		if( len( instance.methods ) AND listFindNoCase( instance.methods, name ) ){
			return true;
		}
		// regex
		if( len( instance.regex ) AND reFindNoCase( instance.regex, name ) ){
			return true;
		}
		// returns
		if( len( instance.returns ) AND instance.returns EQ returns ){
			return true;
		}
		// annotation
		if( len( instance.annotation ) AND structKeyExists( arguments.metadata, instance.annotation )){
			// No annotation value
			if( NOT structKeyExists( instance,"annotationValue" ) ){ return true; }
				
			// check annotation value
			if( structKeyExists( instance,"annotationValue" ) AND arguments.metadata[ instance.annotation ] EQ instance.annotationValue ){
				return true;	
			}
		}
	
		return false;   
	} // matchMethodRules()

	/*
	* @hint		Go through all the rules in this matcher and match
	* @output 	false
	*
	* @target.hint				The target to match against to
	* @mapping.hint				The target mapping to match against to
	* @mapping.colddoc:generic	coldbox.system.ioc.config.Mapping
	*/
	private boolean function matchClassRules(
		required any target,
		required any mapping
	){
		var md	  = arguments.mapping.getObjectMetadata();
		var path  = reReplace( md.name, "(\/|\\)", ".", "all" );
		
		// Start with any()
		if( instance.any ){ return true; }
		// Check explicit mappings
		if( len( instance.mappings ) AND listFindNoCase( instance.mappings, arguments.mapping.getName() ) ){
			return true;
		}
		// regex
		if( len( instance.regex ) AND reFindNoCase( instance.regex, path ) ){
			return true;
		}
		// instanceOf
		if( len( instance.instanceOf ) AND isInstanceOf( arguments.target, instance.instanceOf )){
			return true;
		}
		// annotation
		if( len( instance.annotation ) AND structKeyExists( md, instance.annotation )){
			// No annotation value
			if( NOT structKeyExists( instance,"annotationValue" ) ){ return true; }
				
			// check annotation value
			if( structKeyExists( instance, "annotationValue" ) AND md[ instance.annotation ] EQ instance.annotationValue ){
				return true;	
			}
		}
		
		return false;   

	} // matchClassRules()
}